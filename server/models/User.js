...
const updateProfile = async (userId, fullname, gender) => {
  const query = 'UPDATE users SET fullname = $1, gender = $2 WHERE user_id = $3 RETURNING *';
  const values = [fullname, gender, userId];
  const res = await pool.query(query, values);
  return res.rows[0];
}
...
const addAddress = async (userId, data, isDefault = false) => {
  ...
  try {
    ...
    const query = 'INSERT INTO addresses (user_id, city, district, ward, street, is_default, name, phone_number) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *';
    const values = [userId, data.city, data.district, data.ward, data.street, isDefault, data.name, data.phone];
...
const updateAddress = async (user_id, address_id, data, isDefault = false) => {
  ...
  try {
    ...
    if (isDefault) {
      ...
      const query = 'UPDATE addresses SET city = $1, district = $2, ward = $3, street = $4, name = $7, phone_number = $8 WHERE address_id = $5 AND user_id = $6 RETURNING *';
