...
const AdminSidebar = ({ activeTab, setActiveTab, isOpen, onClose }) => {
  ...
  return (
    <aside className={`w-64 bg-[#004535] text-white flex flex-col fixed h-full shadow-2xl z-30 transform transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        <button onClick={() => handleTabClick('dashboard')} className={menuClass('dashboard')}>
          <LayoutDashboard size={20} /> Dashboard
        </button>

        <p className="px-4 text-xs font-semibold text-white/40 uppercase mt-6 mb-2">Quản lý</p>
        <button onClick={() => handleTabClick('users')} className={menuClass('users')}>
          <Users size={20} /> Người dùng
        </button>
        <button onClick={() => handleTabClick('products')} className={menuClass('products')}>
