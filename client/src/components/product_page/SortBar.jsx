...
const SortBar = ({ sortOption, setSortOption }) => {
  ...
  const getBtnClass = (option) => {
     const base = "px-4 py-1.5 rounded text-xs font-medium transition-colors border";
  ...
  return (
    <div className="bg-white p-3 rounded-lg shadow-sm mb-4 flex flex-wrap gap-2 items-center">
        ...
        <button onClick={() => setSortOption('default')} className={getBtnClass('default')}>
            <Filter size={12} className="inline mr-1"/> Liên quan
        </button>
        
        {/* Giả định: Mockdata ID lớn là mới nhất */}
        <button onClick={() => setSortOption('newest')} className={getBtnClass('newest')}>
            Mới nhất
        </button>
